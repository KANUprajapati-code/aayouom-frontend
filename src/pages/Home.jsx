...
const Home = () => {
  ...
  const activeBanners = cms?.heroBanners?.length > 0
    ...
    : [{
      ...
      title1: "Premium Healthcare",
      title2: "Solutions",
      description: "Professional procurement platform."
    }];
  ...
  return (
    <div className="space-y-20 pb-20 bg-white">
      ...
      {/* 🔥 HERO SECTION FIXED */}
      <section className="relative w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-4">
        <div className="relative rounded-[20px] md:rounded-[40px] overflow-hidden shadow-xl bg-white">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {/* ✅ IMAGE FIX (NO CROP) */}
              <div
                className={`w-full h-[250px] sm:h-[320px] md:h-[420px] lg:h-[520px] bg-no-repeat bg-cover ${currentSlide === 0
                    ? "bg-right md:bg-[80%_center]"     // product वाला banner
                    : "bg-center"                      // doctor वाला banner
                  }`}
                style={{
              ...
              {/* CONTENT */}
              <div className="absolute inset-0 flex flex-col justify-center px-6 md:px-16 max-w-2xl text-white space-y-4">

                <h1 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl font-bold">
                  {activeBanners[currentSlide].title1}<br />
                  <span className="text-blue-400">{activeBanners[currentSlide].title2}</span>
                </h1>
                ...
                <Link
                  to="/products"
                  className="bg-blue-600 px-6 py-3 rounded-lg w-fit hover:bg-blue-700 transition"
                >
                  Explore →
                </Link>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[1, 2, 3].map(i => (
            <div key={i}>
              ...
              <p className="text-sm text-gray-500">Trusted supply chain</p>
            </div>
          ))}
        </div>
      </section>
